//Create a Date object and alert the current date and time.

// const d = new Date();
// alert(d);

// extract the year (four digits) out of a date object.

// const d = new Date();
// year = d.getFullYear();
// alert(d);

// get the month (0-11) out of a date object.

// const d = new Date();
// month = d.getMonth();

// set the year of a date object to 2020.

// const d = new Date();
// d.setFullYear(2020);

//js Math
    //let loop

// create a random number.

let r = Math.random();
console.log (r)// displays random number between o-1 every time the page is refreshed.

//Use the correct Math method to return the largest number of 10 and 20.

// let x = Math.max(10, 20);
// console.log(x)

//return smallest no.
let z = Math.min(2, 20);
console.log(z)

//round a number to the nearest integer.

// let x = Math.round(5.3);
// console.log(x)

//get the square root of 9.

let x = Math.sqrt(9);
console.log(x)